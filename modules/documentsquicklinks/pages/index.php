...
if(checkSession()) {
	...
	if(checkPermission($module)) {
		...
		require($root.'/includes/common/set-pagination-users.php');
		...
		<div class="content-wrapper">
			...
			<section class="content-header">
				<div class="container-fluid">
					...
					<div class="row mb-2">
						<div class="col-sm-6">
							<h1><i class="nav-icon far fa-image mr-3"></i><?php echo 'Documents & Quick Link'; ?> 	(Max of 20 only)</h1>
						</div>
					</div>
				...
				<div class="container-fluid">
					...
					<div class="card-header">
							...
							<div class="card-tools">
								<?php 
								    $sql = $pdo->prepare("SELECT count('docu_name') as total FROM documentsquicklinks ");
									$sql->execute();
									$data = $sql->fetch(PDO::FETCH_ASSOC);
									$total = $data['total'];
									if($total != 20 ) { 
										echo '<a href="add-documents-and-quick-link" class="btn btn-primary btn-md">';
										echo '<i class="fa fa-plus mr-2"></i>';
										echo 'Add More</a>';
									} elseif ($total == 20 ){   ?>
									<a href="#" style="cursor: not-allowed;" class="btn btn-danger btn-md">
										<?php echo $total; ?>
										You already have 20 Downloaded Documents
									</a>
								<?php } ?>
							</div>
						</div>
									...
									<tbody>
										...
										if($sql->rowCount() == 0) {
											...
											while($data = $sql->fetch(PDO::FETCH_ASSOC)) {
												...
												echo '>';
													// title
													echo '<td><a href="/edit-'.$prefix.'/'.$process.'/'.$id.'">'.$data['docu_name'].'</a></td>';

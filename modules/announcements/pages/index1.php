<?php
// INCLUDES
$module = 'announcements'; $prefix = 'announcement'; $process = 'list';
require($_SERVER['DOCUMENT_ROOT'].'/includes/config.php');
require($_SERVER['DOCUMENT_ROOT'].'/includes/unsetSession.php');
unset($_SESSION['add_announcement']);

$sql = $pdo->prepare("SELECT * FROM pagination_set_active WHERE id = 3");
$sql->execute();
...
if(checkSession()) {
	...
	if(checkPermission($module)) {
		...
		$fields_arr = array('announcment_title','date_edit');
		$search_placeholder = 'Title, Date Edited';
...
<head>
	...
	<title><?php echo 'Announcements' ?> &middot; <?php echo renderLang($sitename); ?></title>
					...
					<div class="card-header">
						<form action="/set-pagination" method="post" style="float:left;">
							...
							<div class="card-tools">
								<a href="add-announce_ments" class="btn btn-primary btn-md">
						...
						<div class="card-body" >
							...
							<div class="table-responsive">
								<table class="table table-data table-bordered table-striped table-hover">
									<thead>
										<tr>
											<th><?php echo 'Announcement Title' ?></th>
											<th><?php echo 'Announcement Images' ?></th>
											<th><?php echo 'last Date Edited' ?></th>
									...
									<tbody>
										...
										if($sql->rowCount() == 0) {
											...
											while($data = $sql->fetch(PDO::FETCH_ASSOC)) {
												...
												echo '>';
													// title
													echo '<td><a href="#">'.$data['announcment_title'].'</a></td>';

													// Image
													echo '<td style="width:250px;">
													<img src="/assets/uploadimages/'.$data['announcment_img'].'" style="width:100%;height:200px;" alt="'.$data['announcment_img'].'">
													</td>';

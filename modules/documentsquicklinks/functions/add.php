...
require($_SERVER['DOCUMENT_ROOT'].'/includes/config.php');
	...
	$sql = $pdo->prepare("SELECT docu_name FROM `documentsquicklinks` WHERE docu_name =?");
	...
	if( $sql->rowCount() > 0 ) { # If rows are found for query
		...
		$_SESSION['add_document_success'] = 'add_document_success';
			...
			$sql = $pdo->prepare("INSERT INTO documentsquicklinks (`user_id`, `docu_name`, `date_added`) VALUES (?,?,?)");
		...
		$sql = $pdo->prepare("SELECT * FROM documentsquicklinks WHERE docu_name LIKE :docu_name ");
		$sql->bindParam(':docu_name', $docu_name);
		$sql->execute();
		$data = $sql->fetch(PDO::FETCH_ASSOC);
			...
			for($i = 0; $i < count($_POST['docu_dl']); $i++) {
				...
				$statement->bindParam(2, $docu_name);
				$statement->bindParam(3, $data['id']);

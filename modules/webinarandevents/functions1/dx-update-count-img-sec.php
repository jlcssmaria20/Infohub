...
$sql = $pdo->prepare("SELECT title FROM `webinarandevents` WHERE date_set =?");
$sql->bindParam(1, $_SESSION['setDate']);
$sql->execute();
if( $sql->rowCount() > 0 ) { # If rows are found for query

	$stmt = $pdo->prepare("SELECT * FROM webinarandevents ORDER BY id ASC ");
	$stmt->execute();
	$data3 = $stmt->fetch(PDO::FETCH_ASSOC);


	$sql = 'UPDATE webinarandevents SET img_count = :img_count WHERE id=:id LIMIT 1';
	$img_count = 0;
	$statement = $pdo->prepare($sql);
	$statement->bindParam(':img_count', $img_count, PDO::PARAM_INT);
	$statement->bindParam(':id', $data3['id'], PDO::PARAM_INT);
	$statement->execute();

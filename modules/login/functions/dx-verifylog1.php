...
if(isset($_POST['submit-login'])) {
	...
	if(strlen($uname) > 0 && strlen($upass) > 0) {
		...
		$user_upass = '';
		$sql = $pdo->prepare("SELECT * FROM users WHERE uname = :uname AND status != 2 AND access_role = 1 LIMIT 1 ");
			...
			case 0:
				...
				if($valid_credentials) {
					...
					if($_SESSION['sys_data']['roleids'] != ',') {
						...
						$sql = $pdo->prepare("SELECT id, permissions FROM roles WHERE id = 2");
						$sql->execute();
						...
						if($_SESSION['sys_data']['photo'] == '') {
							switch($_SESSION['sys_data']['gender']) {
								case 0:
									$_SESSION['sys_data']['photo'] = '/dist/img/avatar5.png';
									break;
								case 1:
									$_SESSION['sys_data']['photo'] = '/dist/img/avatar2.png';
									break;
								default:
									$_SESSION['sys_data']['photo'] = '/assets/images/profile-default.png';
									break;
							}
						} else {
							$_SESSION['sys_data']['photo'] = '/modules/users/assets/images/profile/'.$_SESSION['sys_data']['photo'];
						}
						...
						header('location: /user-admin-login');
